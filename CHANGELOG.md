...
- Exact-SHA source retrieval is Phase 7's, so a gateway installed without a verified source provider advertises only its non-mutating capabilities and refuses `apply` and `retry` as an incomplete bootstrap rather than shipping a placeholder. The gateway is installed on no host yet.
